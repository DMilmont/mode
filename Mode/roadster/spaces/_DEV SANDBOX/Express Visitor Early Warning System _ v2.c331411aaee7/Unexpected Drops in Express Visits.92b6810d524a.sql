...
SELECT 
       to_char(ROW_NUMBER() OVER (ORDER BY ((COALESCE("Previous 7 Days Express Visits", use_if_missing) - "Average Express Visits for 4 Week Period") / (|/"Average Express Visits for 4 Week Period")) asc), '00')  || ' ' || dp.name || ' (' || tabAE.success_manager || ')' "Display String",
       ...
       status,
       ROW_NUMBER() OVER (ORDER BY ((COALESCE("Previous 7 Days Express Visits", use_if_missing) - "Average Express Visits for 4 Week Period") / (|/"Average Express Visits for 4 Week Period")) asc ) "Rank",
...
ORDER BY ((COALESCE("Previous 7 Days Express Visits", use_if_missing) - "Average Express Visits for 4 Week Period") / (|/"Average Express Visits for 4 Week Period")) asc 
;
