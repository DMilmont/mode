...
tDealerCount AS
  (SELECT dealer_name AS "Dealer Partner",
    ...
    LEFT JOIN fact.salesforce_dealer_info dp
      ...
      Group By "Dealer Partner",  "Success Manager", "Account Executive", tReportCount.dpid 
      order by "Report Views" DESC)




SELECT tDealerCount.rank,
       dp.dpid,
       dp.dealer_name AS "Dealer Partner",
       tReportCount.agent_email,
       sum(tReportCount.daily_views) AS "Report Views",
       dp.success_manager as "Success Manager",
       dp.account_executive as "Account Executive",

       dp.dealer_name || '<br/>(' || dp.success_manager || ')' AS "Display Label" 
FROM tReportCount


Left Join tDealerCount
  on tReportCount.dpid = tDealerCount.dpid
...
LEFT JOIN fact.salesforce_dealer_info dp
  ON tReportCount.dpid = dp.dpid
       ...
       where agent_email !~* '@roadster.com\M' 
       and dp.status = 'Live'
       and tDealerCount.rank <= 25
  

Group By dp.dealer_name,  dp.dpid, dp.success_manager, dp.account_executive, tDealerCount.rank, tReportCount.agent_email
order by "rank", "Success Manager"
