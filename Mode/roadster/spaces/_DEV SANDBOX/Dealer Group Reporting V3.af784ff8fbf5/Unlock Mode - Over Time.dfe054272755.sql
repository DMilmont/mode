...
SELECT date "Date",
       dp.name "Dealership",
--        CASE WHEN properties ->> 'in_store_purchase_wizard' = '' THEN 'false'
--             else properties ->> 'in_store_purchase_wizard' END in_store_purchase_wizard,
--        CASE WHEN properties ->> 'in_store_purchase_modal' = '' then 'false'
--             else properties ->> 'in_store_purchase_modal' END in_store_purchase_modal,
       CASE 
        WHEN properties ->> 'price_unlock_mode' = 'nolock' THEN 'Prices Unlocked'
        WHEN properties ->> 'price_unlock_mode' = 'pervin' THEN 'Counted Per-Vin'
        WHEN properties ->> 'price_unlock_mode' = 'msrplock' THEN 'Locked - Price < MSRP'
        WHEN properties ->> 'price_unlock_mode' = 'invoicelock' THEN 'Locked - Price < Invoice'
        WHEN properties ->> 'price_unlock_mode' = 'lock' THEN 'Always Locked'
        ELSE properties ->> 'price_unlock_mode'
        END price_unlock_mode,
       1 ct
