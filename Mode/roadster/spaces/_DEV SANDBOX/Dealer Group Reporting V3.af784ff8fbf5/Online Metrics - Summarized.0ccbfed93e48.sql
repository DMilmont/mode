...
and date_trunc('day', cohort_date_utc) >=  '{{ start_date }}'  
and date_trunc('day', cohort_date_utc) <= '{{ end_date }}' 
GROUP BY 1,2
...
SELECT f_prospect.cohort_date_utc                AS date,
  ...
  FROM fact.f_prospect
  WHERE ((f_prospect.cohort_date_utc >= '{{ start_date }}')) AND
         (f_prospect.cohort_date_utc < '{{ end_date }}') AND (f_prospect.is_in_store IS NOT FALSE) AND
         (f_prospect.item_type = 'OrderStarted' :: text) AND (f_prospect.source = 'Lead Type' :: text)
  GROUP BY 
  f_prospect.cohort_date_utc, f_prospect.dpid, f_prospect.dpsk, 'Online Orders' :: text
), 
online_orders as (
select 
  d.date
...
SELECT f_prospect.cohort_date_utc                AS date,
  ...
  FROM fact.f_prospect
  WHERE ((f_prospect.cohort_date_utc >= '{{ start_date }}')) AND
         (f_prospect.cohort_date_utc < '{{ end_date }}' ) AND (f_prospect.is_in_store <> FALSE)
  GROUP BY 1,2,3
)
...
SELECT "Date",
"Dealership",
CASE WHEN "Dealership" NOT ILIKE '%Percentile%' THEN COALESCE(ROUND(AVG("Dealer Visitors"), 2), 0.0) ELSE 0.0 END "Dealer Visitors",
CASE WHEN "Dealership" NOT ILIKE '%Percentile%' THEN COALESCE(ROUND(AVG("Online Express Visitors"), 2), 0.0) ELSE 0.0 END "Express Visitors",
...
SELECT "Dealership", 
SUM("Dealer Visitors") "Dealer Visitors",
SUM("Express Visitors") "Express Visitors", 
AVG("Online Express Ratio") "Online Express Ratio",
AVG("Prospect Conversion") "Prospect Conversion",
SUM("Online Shares") "Online Shares",
SUM("Online Orders") "Online Orders", 
SUM("Roadster Matched Sales") "Roadster Matched Sales", 
