...
SELECT dpid,a.department,a.email,a.agent_id ,
       SUM("Service Plans Completed") AS "Service Plans Completed",
        ...
        SUM(case when is_in_store = false then "Order Submitted" else null end ) AS "Online Orders Submitted",
        SUM(case when is_in_store = true then "Order Submitted" else null end ) AS "InStore Orders Submitted",
