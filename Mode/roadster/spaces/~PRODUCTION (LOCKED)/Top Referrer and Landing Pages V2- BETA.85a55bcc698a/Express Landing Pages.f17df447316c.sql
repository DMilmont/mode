WITH last_month_visits as ( SELECT  dpid
                          ,coalesce(express_landing_page_path,'n/a') express_landing_page_path
...
detail as (select a.month_year 
       ...
       ,a.dpid
       ,case when b.rnk<=10 then coalesce(a.express_landing_page_path,'n/a') else 'other' end as express_landing_page_path
...
left join last_month_visits b on coalesce(a.express_landing_page_path,'n/a')=b.express_landing_page_path and a.dpid=b.dpid 
...
and a.month_year <= (date_trunc('day', now()) - INTERVAL '1 Months')
 and is_in_store is false
group by 1,2,3,4,5,6,mt.cnt,od.month_year 
