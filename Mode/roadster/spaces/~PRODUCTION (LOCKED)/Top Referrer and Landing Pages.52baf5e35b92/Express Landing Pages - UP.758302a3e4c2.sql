...
inner join last_month_visits b on coalesce(a.express_landing_page_path,'x')=coalesce(b.express_landing_page_path,'x') and a.dpid=b.dpid and  b.rnk<=5
inner join monthly_totals mt on a.month_year=mt.month_year
group by 1,2,3,4,mt.cnt
),
unpivot as (select d.month_year
      ,d.dpid 
      ,d.express_landing_page_path
      ,d.rnk_page
      ,e.*
from detail d
, LATERAL (
...
select month_year + 1 as month_year
      ,dpid
      ,COALESCE(express_landing_page_path,'n/a') as express_landing_page_path
      ,calc_type
