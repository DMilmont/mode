...
output_data as (
  SELECT
         to_char(ROW_NUMBER() OVER (PARTITION BY tabAE.success_manager ORDER BY ((COALESCE("Current Week", use_if_missing) - "Previous Month - Weekly Average") / (|/"Previous Month - Weekly Average")) asc ), '00')  || ' ' || dp.dpid "Display String" ,
         dp.name "Dealer Partner",
        ROUND("Previous Month - Weekly Average"::numeric,0) "Previous Month - Weekly Average",
        ROUND(COALESCE("Current Week"::numeric, use_if_missing),0) "Current Week",
         CASE
         WHEN ((COALESCE("Current Week", use_if_missing) - "Previous Month - Weekly Average") / (|/"Previous Month - Weekly Average"))::decimal <=-2.3 THEN '#8a1a23'
         WHEN ((COALESCE("Current Week", use_if_missing) - "Previous Month - Weekly Average") / (|/"Previous Month - Weekly Average"))::decimal <=0 THEN '#3e454e'
         ...
         ROW_NUMBER() OVER (PARTITION BY tabAE.success_manager ORDER BY ((COALESCE("Current Week", use_if_missing) - "Previous Month - Weekly Average") / (|/"Previous Month - Weekly Average")) asc ) "Rank",
  ...
  WHERE status = 'Live'
    ...
    and (tabAE.success_manager = '{{DSM}}')

  UNION

  SELECT
         to_char(ROW_NUMBER() OVER (PARTITION BY tabAE.account_executive ORDER BY ((COALESCE("Current Week", use_if_missing) - "Previous Month - Weekly Average") / (|/"Previous Month - Weekly Average")) asc ), '00')  || ' ' || dp.dpid "Display String" ,
         dp.name "Dealer Partner",
        ROUND("Previous Month - Weekly Average"::numeric,0) "Previous Month - Weekly Average",
        ROUND(COALESCE("Current Week"::numeric, use_if_missing),0) "Current Week",
         CASE
         ...
         tabAE.health_score,
         tabAE.off_weeks_since_launch,
         ROW_NUMBER() OVER (PARTITION BY tabAE.account_executive ORDER BY ((COALESCE("Current Week", use_if_missing) - "Previous Month - Weekly Average") / (|/"Previous Month - Weekly Average")) asc ) "Rank",
         ((COALESCE("Current Week", use_if_missing) - "Previous Month - Weekly Average") / (|/"Previous Month - Weekly Average"))::decimal "Importance"
  FROM (
      SELECT DISTINCT dpid, name, 0 use_if_missing
      FROM dealer_partners
  ...
  WHERE status = 'Live'
    and off_weeks_since_launch > 6
    and (tabAE.account_executive = '{{DSM}}')
