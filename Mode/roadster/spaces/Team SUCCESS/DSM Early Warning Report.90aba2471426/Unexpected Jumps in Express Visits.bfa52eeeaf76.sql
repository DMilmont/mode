...
SELECT 
       to_char(ROW_NUMBER() OVER (ORDER BY ((COALESCE("Current Week", use_if_missing) - "Previous Month - Weekly Average") / (|/"Previous Month - Weekly Average")) desc ), '00')  || ' ' || dp.dpid ||  '</a><br/>' || tabAE.success_manager  "Display String",
       ...
       ROW_NUMBER() OVER (ORDER BY ((COALESCE("Current Week", use_if_missing) - "Previous Month - Weekly Average") / (|/"Previous Month - Weekly Average")) desc ) "Rank",
